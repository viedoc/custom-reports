...
if (ncol(qry) == 0 || nrow(qry) == 0) {
  ...
  if (nrow(manualQry) == 0) reportOutput <- list("data" = list("data" = output))
    ...
    formLevel <- countManualQueries(manualQry, c('StudyName', 'Country', 'SiteCode', 'SiteName', 'EventName', 'EventSeq', 'SubjectSeq', 'SubjectId', 'FormId', 'FormName', 'FormSeq')) %>%
      data.frame() %>%
      mutate(
    ...
    formLevel <- setLabel(formLevel, list("Study", "Country", "Site Code", "Site Name", "Event", "Event Sequence", "Subject Sequence", "Subject", "Form", "Form Sequence", "Total", "# of Manual Queries (Raised)", "# of Manual Queries (Resolved)", "# of Manual Queries (Rejected)", "# of Manual Queries (Approved)", "# of Manual Queries (Closed)", "# of Manual Queries (Removed)", "Resulting in Data Changes", "Updates / Query %", "Queries / Item %", "# of Queries Open > 7 days", "# of Queries Open > 14 days", "# of Queries Open > 21 days", "Average Time to Resolution (days)", "Average Time to Approval (days)"))
    headerForm <- list(
      firstLevel = c("Study", "Country", "Site Code", "Site Name", "Event", "Event Sequence", "Subject Sequence", "Subject", "Form", "Form Sequence", "Total", rep("# of Manual Queries", 6), "Resulting in Data Changes", rep("Ratio (%)", 2), rep("# of Queries Open", 3), rep("Average time to (days)", 2)),
    ...
    subjectLevel <- countManualQueries(manualQry, c('StudyName', 'Country', 'SiteCode', 'SiteName', 'SubjectSeq', 'SubjectId')) %>%  
      data.frame() %>%
      mutate(
        processedQries = rowSums(across(c(resolvedQueries, rejectedQueries, approvedQueries, closedQueries))),
        updatesRatio = ifelse(processedQries != 0, round(updatedQueries * 100  / processedQries, 2), 0),
    ...
    subjectLevel <- setLabel(subjectLevel, list("Study", "Country", "Site Code", "Site Name", "Subject Sequence", "Subject", "Total", "# of Manual Queries (Raised)", "# of Manual Queries (Resolved)", "# of Manual Queries (Rejected)", "# of Manual Queries (Approved)", "# of Manual Queries (Closed)", "# of Manual Queries (Removed)", "Resulting in Data Changes", "Updates / Query %", "Queries / Item %", "# of Queries Open > 7 days", "# of Queries Open > 14 days", "# of Queries Open > 21 days", "Average Time to Resolution (days)", "Average Time to Approval (days)"))
    headerSubject <- list(
      firstLevel = c("Study", "Country", "Site Code", "Site Name", "Subject Sequence", "Subject", "Total", rep("# of Manual Queries", 6), "Resulting in Data Changes", rep("Ratio (%)", 2), rep("# of Queries Open", 3), rep("Average time to (days)", 2)),
      secondLevel = c("Raised", "Resolved", "Rejected", "Approved", "Closed", "Removed", "Updates / Query", "Queries / Item", "> 7 days", "> 14 days", "> 21 days", "Resolution", "Approval")
    )
