...
if (ncol(qry) == 0 || nrow(qry) == 0) {
  ...
  countValQueries <- function(query, groupby_cols) {
    qryValue <- query %>%
  ...
  # Get all the Validation queries ----
  valQry <- qry %>% filter(( QueryType == "Validation" ) & !grepl("^Prequery", QueryStatus))  
  if (nrow(valQry) == 0) reportOutput <- list("data" = list("data" = output))
  else {
    # Get Study Name ----
    valQry$SiteCode <- as.character(valQry$SiteCode)
    valQry <- valQry %>%
    ...
    eventLevel <- countValQueries(valQry, c('StudyName', 'Country', 'SiteCode', 'SiteName', 'EventName')) %>%
      data.frame() %>%
      mutate(
        processedQries = rowSums(across(c(resolvedQueries, rejectedQueries, approvedQueries, closedQueries))),
        updatesRatio = ifelse(processedQries != 0, round(updatedQueries * 100  / processedQries, 2), 0),
    ...
    eventLevel <- setLabel(eventLevel, list("Study", "Country", "Site Code", "Site Name", "Event", "Total", "# of Validation Queries (Raised)", "# of Validation Queries (Resolved)", "# of Validation Queries (Rejected)", "# of Validation Queries (Approved)", "# of Validation Queries (Closed)", "# of Validation Queries (Removed)", "Resulting in Data Changes", "Updates / Query %", "Queries / Item %", "# of Queries Open > 7 days", "# of Queries Open > 14 days", "# of Queries Open > 21 days", "Average Time to Resolution (days)", "Average Time to Approval (days)"))
    headerEvent <- list(
      firstLevel = c("Study", "Country", "Site Code", "Site Name", "Event", "Total", rep("# of Validation Queries", 6), "Resulting in Data Changes", rep("Ratio (%)", 2), rep("# of Queries Open", 3), rep("Average time to (days)", 2)),
      secondLevel = c("Raised", "Resolved", "Rejected", "Approved", "Closed", "Removed", "Updates / Query", "Queries / Item", "> 7 days", "> 14 days", "> 21 days", "Resolution", "Approval")
    )
    ...
    siteLevel <- countValQueries(valQry, c('StudyName', 'Country', 'SiteCode', 'SiteName')) %>%  
      data.frame() %>%
      mutate(
        processedQries = rowSums(across(c(resolvedQueries, rejectedQueries, approvedQueries, closedQueries))),
        updatesRatio = ifelse(processedQries != 0, round(updatedQueries * 100  / processedQries, 2), 0),
    ...
    siteLevel <- setLabel(siteLevel, list("Study", "Country", "Site Code", "Site Name", "Total", "# of Validation Queries (Raised)", "# of Validation Queries (Resolved)", "# of Validation Queries (Rejected)", "# of Validation Queries (Approved)", "# of Validation Queries (Closed)", "# of Validation Queries (Removed)", "Resulting in Data Changes", "Updates / Query %", "Queries / Item %", "# of Queries Open > 7 days", "# of Queries Open > 14 days", "# of Queries Open > 21 days", "Average Time to Resolution (days)", "Average Time to Approval (days)", "Number of Subjects", "Queries / Subject", "% of Queries in Trial", "% of Queries in Country"))
    headerSite <- list(
      firstLevel = c("Study", "Country", "Site Code", "Site Name", "Total", rep("# of Validation Queries", 6), "Resulting in Data Changes", rep("Ratio (%)", 2), rep("# of Queries Open", 3), rep("Average time to (days)", 2), "Number of Subjects", "Queries / Subject", "% of Queries in Trial", "% of Queries in Country"),
      secondLevel = c("Raised", "Resolved", "Rejected", "Approved", "Closed", "Removed", "Updates / Query", "Queries / Item", "> 7 days", "> 14 days", "> 21 days", "Resolution", "Approval")
    )
    ...
    countryLevel <- countValQueries(valQry, c('StudyName', 'Country')) %>%  
      data.frame() %>%
      mutate(
        processedQries = rowSums(across(c(resolvedQueries, rejectedQueries, approvedQueries, closedQueries))),
        updatesRatio = ifelse(processedQries != 0, round(updatedQueries * 100  / processedQries, 2), 0),
    ...
    countryLevel <- setLabel(countryLevel, list("Study", "Country", "Total", "# of Validation Queries (Raised)", "# of Validation Queries (Resolved)", "# of Validation Queries (Rejected)", "# of Validation Queries (Approved)", "# of Validation Queries (Closed)", "# of Validation Queries (Removed)", "Resulting in Data Changes", "Updates / Query %", "Queries / Item %", "# of Queries Open > 7 days", "# of Queries Open > 14 days", "# of Queries Open > 21 days", "Average Time to Resolution (days)", "Average Time to Approval (days)", "Number of Subjects", "Queries / Subject", "% of Queries in Trial"))
    headerCountry <- list(
      firstLevel = c("Study", "Country", "Total", rep("# of Validation Queries", 6), "Resulting in Data Changes", rep("Ratio (%)", 2), rep("# of Queries Open", 3), rep("Average time to (days)", 2), "Number of Subjects", "Queries / Subject", "% of Queries in Trial"),
      secondLevel = c("Raised", "Resolved", "Rejected", "Approved", "Closed", "Removed", "Updates / Query", "Queries / Item", "> 7 days", "> 14 days", "> 21 days", "Resolution", "Approval")
    )
    ...
    mostValQueried <- valQry %>%
      select(StudyName, FormId, FormName, ItemId, ItemName, RangeCheckOID, QueryText, ClosedByDataEdit, QueryStatus) %>%
      group_by(StudyName, FormId, FormName, ItemId, ItemName, RangeCheckOID, QueryText) %>%
      summarise(
        totalQueries = n(),
        updatedQueries = sum(ClosedByDataEdit == "Yes", na.rm = T),
        openQueries = sum(QueryStatus == "Query Raised"),
    ...
    mostValQueried[is.na(mostValQueried$FormName), "FormName"] <- "Event Date"
    mostValQueried <- mostValQueried %>%
      select(StudyName, FormId, FormName, ItemId, ItemName, RangeCheckOID, QueryText, totalQueries, openQueries, updatedQueries) %>%
      arrange(desc(totalQueries))
    mostValQueried <- prepareDataForDisplay(mostValQueried)
    mostValQueried <- setLabel(mostValQueried, list("Study", "FormId", "Form", "ItemId", "Item", "Range check OID", "Edit Check", "Total # of Raised Queries", "# of Open Queries", "# of Queries Resulting in Data Change"))
    widths <- rep(0, ncol(mostValQueried))
    mostValQueriedColumnDefs <- getColumnDefs(colwidths = widths, alignRight = c(8:10))
    ...
    reportOutput <- list(
      ...
      "Most Triggered Validation Queries" = list("data" = mostValQueried, "columnDefs" = mostValQueriedColumnDefs)
    )
  }
  }
