...
if(!is.null(vs)){
  ...
  vs <- data.frame(matrix(ncol = 7, nrow = 0))
  colnames(vs) <- c("sysBPOutliers", "SiteName", "SiteCode", "SubjectId", "EventName", "EventDate", "SYSBP_VSORRES")
}
...
if(nrow(sysBPOutliers)>0){
  ...
  sysOut_footerText = paste("No data")
}

if(nrow(diaBPOutliers)>0){
  diaBPOutliers <- select(sysBPOutliers, SiteName, SiteCode, SubjectId, EventName, EventDate, SYSBP_VSORRES)
  ...
  colnames(diaBPOutliers) <- c("sysBPOutliers", "SiteName", "SiteCode", "SubjectId", "EventName", "EventDate", "SYSBP_VSORRES")
  diaOut_footerText = paste("No data")
}
